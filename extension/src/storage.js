/* SidePrompt – shared data layer (classic script, exposes global `PV`) */
const PV = (() => {
  const KEY = 'pv_data';
  ...
  async function exportJSON() {
    const data = await load();
    return JSON.stringify({ app: 'SidePrompt', version: 1, exportedAt: new Date().toISOString(), prompts: data.prompts, folders: data.folders }, null, 2);
  }
