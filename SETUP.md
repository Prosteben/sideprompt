...
2. ✅ HOTOVO – rozšíření je registrované pod ID `promptvault-drzymalla` (ID je trvalé, v kódu je v `EXTPAY_ID`). Zobrazovaný název (Extension Name) nastav na **SidePrompt**.
...
1. Založ účet na https://github.com (pokud nemáš) a vytvoř **veřejný** repozitář, např. `sideprompt`.
...
4. Výslednou URL (např. `https://<tvůj-login>.github.io/sideprompt/privacy-policy.html`) **napiš mi** – vložím ji do `extension/src/options.js` (`PRIVACY_URL`) a znovu vytvořím ZIP.

## 5. Publikace (klikací část, ~10 min)
1. Spusť `npm run release` (nebo mě požádej) → vznikne `dist/sideprompt-1.0.0.zip`.
