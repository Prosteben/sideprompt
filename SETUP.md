...
2. **Register extension** → Extension ID zadej přesně: `promptvault`
   - Pokud je ID obsazené, zvol jiné (např. `promptvault-ai`) a **napiš mi ho** – změním konstantu `EXTPAY_ID` v `extension/src/storage.js`.
3. V nastavení rozšíření na ExtensionPay vytvoř plán: **One-time payment, 9.99 USD** (doporučeno; případně 7.99 USD).
...
1. Založ účet na https://github.com (pokud nemáš) a vytvoř **veřejný** repozitář, např. `promptvault`.
...
4. Výslednou URL (např. `https://<tvůj-login>.github.io/promptvault/privacy-policy.html`) **napiš mi** – vložím ji do `extension/src/options.js` (`PRIVACY_URL`) a znovu vytvořím ZIP.

## 5. Publikace (klikací část, ~10 min)
1. Spusť `npm run release` (nebo mě požádej) → vznikne `dist/promptvault-1.0.0.zip`.
